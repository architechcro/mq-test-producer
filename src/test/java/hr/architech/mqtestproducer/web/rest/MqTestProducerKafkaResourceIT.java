...
import hr.architech.mqtestproducer.config.KafkaProperties;
...
class MqTestProducerKafkaResourceIT {
    ...
    void setup() {
        ...
        MqTestProducerKafkaResource kafkaResource = new MqTestProducerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/mq-test-producer-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/mq-test-producer-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
